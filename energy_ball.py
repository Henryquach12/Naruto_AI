import cv2
import numpy as np
import math


def _blend(frame, layer, alpha):
    """Safe in-place blend: frame = alpha*layer + (1-alpha)*frame."""
    frame[:] = cv2.addWeighted(layer, alpha, frame, 1.0 - alpha, 0)


class EnergyBall:
    """Renders a Rasengan-style blue energy ball on a frame."""

    # BGR colours  (blue channel is HIGH → vivid blue ball)
    CORE_COLOR   = (255, 255, 255)   # white hot core
    INNER_COLOR  = (255, 220,  80)   # bright cyan-white
    MID_COLOR    = (255, 140,  20)   # deep blue
    OUTER_COLOR  = (180,  60,   0)   # dark-blue outer glow
    AURA_COLOR   = (120,  20,   0)   # wide dark-blue halo
    SPARK_COLOR  = (255, 200, 100)   # spark streaks
    RING_COLOR   = (255, 180,  60)   # segmented outer ring

    def __init__(self):
        self.tick = 0
        self.spiral_particles = self._init_spiral_particles(60)
        self.spark_particles  = self._init_spark_particles(30)

    # ── particle init ─────────────────────────────────────────────────────────
    def _init_spiral_particles(self, n):
        rng = np.random.default_rng(42)
        return [
            {
                "angle":  (2 * math.pi / n) * i,
                "radius": float(rng.uniform(0.55, 0.90)),
                "speed":  float(rng.uniform(0.04, 0.10)),
                "layer":  int(rng.integers(0, 2)),
                "size":   int(rng.integers(2, 5)),
            }
            for i in range(n)
        ]

    def _init_spark_particles(self, n):
        rng = np.random.default_rng(7)
        return [
            {
                "angle":  float(rng.uniform(0, 2 * math.pi)),
                "length": float(rng.uniform(0.3, 0.8)),
                "speed":  float(rng.uniform(0.06, 0.15)),
                "offset": float(rng.uniform(0, 2 * math.pi)),
            }
            for _ in range(n)
        ]

    # ── drawing helpers ───────────────────────────────────────────────────────
    def _draw_glow(self, frame, cx, cy, radius, color, max_alpha, scale):
        """Paint one soft glow ring."""
        layer = frame.copy()
        cv2.circle(layer, (cx, cy), int(radius * scale), color, -1)
        _blend(frame, layer, max_alpha)

    def _draw_spiral_particles(self, frame, cx, cy, radius):
        for p in self.spiral_particles:
            p["angle"] += p["speed"]
            arm_offset = math.pi if p["layer"] == 1 else 0.0
            angle = p["angle"] + arm_offset
            wobble = 0.04 * math.sin(self.tick * 0.05 + p["angle"] * 3)
            r = int((p["radius"] + wobble) * radius)
            px = cx + int(r * math.cos(angle))
            py = cy + int(r * math.sin(angle))
            b = int(200 + 55 * math.sin(p["angle"]))
            g = int(100 + 80 * math.cos(p["angle"] * 0.5))
            cv2.circle(frame, (px, py), p["size"],        (b, g, 50), -1)
            cv2.circle(frame, (px, py), max(1, p["size"] - 1), (255, 240, 200), -1)

    def _draw_spark_streaks(self, frame, cx, cy, radius):
        for p in self.spark_particles:
            p["angle"] += p["speed"]
            base_r = int(radius * 0.6)
            tip_r  = int(radius * (0.95 + 0.15 * math.sin(self.tick * 0.07 + p["offset"])))
            x1 = cx + int(base_r * math.cos(p["angle"]))
            y1 = cy + int(base_r * math.sin(p["angle"]))
            x2 = cx + int(tip_r  * math.cos(p["angle"]))
            y2 = cy + int(tip_r  * math.sin(p["angle"]))
            cv2.line(frame, (x1, y1), (x2, y2), self.SPARK_COLOR,
                     max(1, int(1 + p["length"])), cv2.LINE_AA)

    def _draw_outer_ring(self, frame, cx, cy, radius):
        """Rotating segmented arc ring — drawn onto a single layer then blended once."""
        segments = 24
        rot = self.tick * 0.03
        ring_layer = frame.copy()
        for i in range(segments):
            a1 = rot + (2 * math.pi / segments) * i
            a2 = a1 + (2 * math.pi / segments) * 0.55
            r_in  = int(radius * 1.05)
            r_out = int(radius * 1.28)
            pts = (
                [[cx + int(r_out * math.cos(a)), cy + int(r_out * math.sin(a))]
                 for a in np.linspace(a1, a2, 6)]
                + [[cx + int(r_in  * math.cos(a)), cy + int(r_in  * math.sin(a))]
                   for a in np.linspace(a2, a1, 6)]
            )
            cv2.fillPoly(ring_layer, [np.array(pts, dtype=np.int32)], self.RING_COLOR)
        ring_alpha = 0.55 + 0.20 * math.sin(self.tick * 0.04)
        _blend(frame, ring_layer, ring_alpha)

    # ── public API ────────────────────────────────────────────────────────────
    def draw(self, frame, center, radius):
        """Composite the Rasengan onto `frame` at `center` (modifies in place)."""
        cx, cy = int(center[0]), int(center[1])
        r      = int(radius)
        self.tick += 1

        # 1. wide soft aura
        self._draw_glow(frame, cx, cy, r, self.AURA_COLOR,  0.20, 2.3)
        # 2. outer glow layers
        self._draw_glow(frame, cx, cy, r, self.OUTER_COLOR, 0.35, 1.9)
        self._draw_glow(frame, cx, cy, r, self.MID_COLOR,   0.40, 1.55)
        self._draw_glow(frame, cx, cy, r, self.INNER_COLOR, 0.45, 1.20)
        # 3. rotating segmented ring
        self._draw_outer_ring(frame, cx, cy, r)
        # 4. spiral particles
        self._draw_spiral_particles(frame, cx, cy, r)
        # 5. spark streaks
        self._draw_spark_streaks(frame, cx, cy, r)
        # 6. main filled ball (semi-transparent)
        ball_layer = frame.copy()
        cv2.circle(ball_layer, (cx, cy), r, self.MID_COLOR, -1)
        _blend(frame, ball_layer, 0.70)
        # 7. inner bright ring (solid)
        cv2.circle(frame, (cx, cy), int(r * 0.70), self.INNER_COLOR, -1)
        # 8. white-hot core (solid)
        cv2.circle(frame, (cx, cy), int(r * 0.35), self.CORE_COLOR,  -1)
        # 9. specular highlight
        cv2.circle(frame, (cx - int(r * 0.22), cy - int(r * 0.22)),
                   int(r * 0.15), (255, 255, 255), -1)
        # 10. edge rim
        cv2.circle(frame, (cx, cy), r, self.INNER_COLOR, 2, cv2.LINE_AA)

        return frame
